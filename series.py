...
def execute_write(query, params=None, wait=False):
        ...
        with col_del:
            if len(df_inv) > 0:
                ...
                if st.button("🗑 Eliminar Fila", use_container_width=True):
                    ...
                    st.rerun()
        if df_inv.empty:
            st.info("📋 La tabla está vacía. Agrega filas.")
        else:
            ...
            if t["estado"] == "pendiente":
                ...
                if t["actividad_id"].lower() == "evidencia":
                    ...
                    with st.form(f"ser_{t['id']}"):
                        st.markdown("Ingresa los seriales de cada componente:", unsafe_allow_html=True)
