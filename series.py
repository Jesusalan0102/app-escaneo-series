...
# 1. Definición de Variables Globales (Evita el NameError)
...
if menu == "📊 Dashboard Ejecutivo":
    ...
    if asig:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Completadas": st.column_config.ProgressColumn("Avance Finalizado", min_value=0, max_value=int(stats_tecnicos['Total'].max() or 1)),
        ...
        st.markdown('<div class="section-title">Detalle General de Actividades</div>', unsafe_allow_html=True)
    ...
    if sols:
        ...
        for s in sols:
            ...
            c1.info(f"**{s['tecnico']}**: {s['actividad_id']} en Unidad {s['unidad']}")
    ...
    st.markdown('<div class="main-header">Captura de Datos Técnicos</div>', unsafe_allow_html=True)
    with st.form("reg_u"):
        c1, c2 = st.columns(2)
    ...
    if not tareas: st.info("No hay tareas asignadas.")
    else:
        for t in tareas:
            with st.expander(f"📦 Unidad {t['unidad']} - {t['actividad_id']}"):
                if t['estado'] == 'pendiente':
                    if st.button(f"Iniciar Trabajo #{t['id']}", use_container_width=True):
                    ...
                    elif st.button(f"Concluir Actividad #{t['id']}", use_container_width=True):
                        execute_write("UPDATE asignaciones SET estado='completada', fecha_fin=NOW() WHERE id=%s", (t['id'],)); st.rerun()
