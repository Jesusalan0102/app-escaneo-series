...
if menu == "📊 Dashboard Ejecutivo":
    ...
    if asig:
        ...
        # Cálculo de porcentaje: (Completadas / Total)
        stats['Productividad'] = (stats['Completadas'] / stats['Total'])
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Productividad": st.column_config.ProgressColumn("Avance Finalizado", min_value=0, max_value=1.0, format="%.0f%%"),
                "En_Curso": "En Curso ⚙️",
        ...
        with c1:
            st.plotly_chart(px.bar(df_a, x='tecnico', color='estado', title="Distribución de Carga por Estado",
                                   labels={'tecnico': 'Técnico', 'estado': 'Estado'},
                                   color_discrete_map={'completada': '#2ECC71', 'en_proceso': '#3498DB', 'pendiente': '#F1C40F'}), use_container_width=True)
        with c2:
            st.plotly_chart(px.pie(df_a, names='estado', title="Estado Global de la Operación", hole=0.5), use_container_width=True)
    ...
    with st.form("manual_form"):
        ...
        if st.form_submit_button("Confirmar Asignación", use_container_width=True):
            ...
            st.success("Asignación completada."); st.rerun()
    ...
    with st.form("reg_u"):
        ...
        if st.form_submit_button("Guardar en Base de Datos", use_container_width=True):
            execute_write(f"INSERT INTO unidades (unit_number, id_lote, {campo}) VALUES (%s,%s,%s) ON DUPLICATE KEY UPDATE {campo}=%s", (unit, lote, valor, valor))
    ...
    if not tareas: st.info("No tienes tareas pendientes por el momento.")
    else:
        for t in tareas:
            with st.expander(f"📦 Unidad: {t['unidad']} - {t['actividad_id']}"):
                if t['estado'] == 'pendiente':
                    if st.button(f"Iniciar Actividad", key=f"st_{t['id']}", use_container_width=True):
