...
def _get_db_config():
    """Configuración de base de datos para todos los entornos."""
...
def _get_conn():
    global _db_lock, _db_conn_holder
    with _db_lock:
    ...
    with col_c:
        ...
        if st.button("Ingresar al Sistema", use_container_width=True, type="primary", key="_login_btn"):
            ...
            if not _u or not _p:
                ...
                if user is None:
                    st.session_state["_login_err"] = "❌ No se pudo conectar a la base de datos. Verifica tu conexión."
                    ...
                    st.session_state.update({"login": True, "user": user[0]["username"], "role": user[0]["role"].lower(), "_login_err": ""})
                        ...
                        if archivos:
                            ...
                            if st.button(f"💾 Guardar {len(archivos)} foto(s)", key=f"savef_{t['id']}", use_container_width=True, type="primary"):
                                ...
                                for i, arc in enumerate(archivos):
                                    arc.seek(0)
                                    ok = execute_write("INSERT INTO evidencias (unit_number, nombre_archivo, contenido, tecnico) VALUES (%s,%s,%s,%s)", (t["unidad"], arc.name, arc.read(), st.session_state.user))
                                    if not ok:
                                        errores += 1
...
# ==================== NUEVA SOLICITUD (Técnico) ====================
    ...
    with st.form("sol_f"):
        ...
        if st.form_submit_button("📤 Enviar Solicitud", use_container_width=True, type="primary"):
            ...
            if activa:
                ...
                etiquetas = {"solicitado": "esperando aprobación del administrador", "pendiente": "pendiente de iniciar", "en_proceso": "actualmente en proceso"}
                ...
                if completada:
                    st.warning(f"⚠️ La actividad **{a_sel}** en la unidad **{unidad_sel}** ya fue completada por **{completada[0]['tecnico']}**. Tu solicitud requiere autorización especial del administrador.")
    ...
    historial = execute_read("SELECT unidad, actividad_id, estado, fecha_inicio, fecha_fin FROM asignaciones WHERE tecnico=%s ORDER BY id DESC LIMIT 20", (st.session_state.user,))
    if historial:
